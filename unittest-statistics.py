...
def main():
    if len(sys.argv) < 2:
        ...
        print('Usage: {} </path/to/test_groestlcoin> [<subtest>]'.format(tool))
        print('For example: {} src/test/test_groestlcoin wallet_tests'.format(tool))
        exit(1)
    test_groestlcoin = sys.argv[1]
    args = [test_groestlcoin, '--log_level=test_suite']
    if len(sys.argv) > 2:
